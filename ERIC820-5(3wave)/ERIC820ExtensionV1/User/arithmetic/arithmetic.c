...
void CalRMS(uint16_t bt)
{
	...
	bt = FIFO_Backpointer(bt, ADC_Period*ADC_N, ADCbufSize);
	buf = 0;	
	for(j=0;j<32;j++){		
...
void CalVerify(uint16_t bt)
{
	...
	bt = FIFO_Backpointer(bt, 10*ADC_Period*ADC_N, ADCbufSize);
	...
	switch(step){
		case 0:
			if(CanComData->FaultFlag){
				CanComData->Faultbt = FIFO_Backpointer(CanComData->Faultbt, (ADC_Period+ADC_Period/4)*ADC_N, ADCbufSize);
				//Test1Set();
				bt = CanComData->Faultbt;
				for(j=0;j<ADC_Period;j++){
					bt1 = FIFO_pointer(bt, ADC_N-1, ADCbufSize);		
					Indata = ADCFifo.AD_Value[bt] - ADCFifo.AD_Value[bt1];
					buf += (Indata*Indata);					
					bt = FIFO_pointer(bt, ADC_N, ADCbufSize);
			...
			break;
		case 1:
			if(FIFO_length(CanComData->Faultbt, AD_pt, ADCbufSize)>=11 * ADC_Period * ADC_N){
				bt = FIFO_Backpointer(CanComData->Faultbt, 9 * ADC_Period * ADC_N, ADCbufSize);
