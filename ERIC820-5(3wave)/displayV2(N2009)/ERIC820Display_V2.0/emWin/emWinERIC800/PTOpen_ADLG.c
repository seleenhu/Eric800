...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  switch (pMsg->MsgId) {
    ...
    //TEXT_SetFont(hItem, GUI_FONT_20B_ASCII);
		...
		{
		case 1:
		TEXT_SetText(hItem,"A相PT断线");
		break;
		case 2:
		TEXT_SetText(hItem,"B相PT断线");
		break;
		case 3:
		TEXT_SetText(hItem,"AB相PT断线");
		break;
		case 4:
		TEXT_SetText(hItem,"C相PT断线");
		break;
		case 5:
		TEXT_SetText(hItem,"AC相PT断线");
    break;
		case 6:
		TEXT_SetText(hItem,"BC相PT断线");
    break;
		default:
		break;
		}
		...
		TEXT_SetTextColor(hItem, GUI_WHITE); 
		TEXT_SetFont(hItem, &XBF_Font24);
		sprintf(s,"全部故障:%d",SDPTFault_sum);
		TEXT_SetText(hItem,s);
		
		hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_8);
		TEXT_SetTextColor(hItem, GUI_WHITE); 
		TEXT_SetFont(hItem, &XBF_Font24);
		sprintf(s,"当前故障:%d",SDPTFault_sum-TA_WavePosition);
		TEXT_SetText(hItem,s);
	 ...
	 switch(SelectWave)
		{
			...
			//======fresh wave
				if(SelectWave!=SelectWaveSave)
				{
					...
					WM_SendMessageNoPara(WM_GetClientWindow(TA_hWin), WM_DisplayWave);
				}
  ...
  case WM_NOTIFY_PARENT:
    ...
    case ID_BUTTON_8: // Notifications sent by 'Resonance Alarm'
      ...
      break;	
			...
			case ID_BUTTON_20: // Notifications sent by 'PT Alarm'
      switch(NCode) {
      case WM_NOTIFICATION_CLICKED:
        // USER START (Optionally insert code for reacting on notification message)
        // USER END
    ...
    case ID_BUTTON_10: // Notifications sent by 'Up'
      ...
      case WM_NOTIFICATION_CLICKED:
        // USER START (Optionally insert code for reacting on notification message)
				if(TA_WavePosition>=(SDPTFault_sum-1))
				{
					break;
				}
				TA_WavePosition++;
...
*       CreateOverVoltage_A
*/
WM_HWIN CreatePTOpen_A(void);
WM_HWIN CreatePTOpen_A(void) {
  //WM_HWIN hWin;
	WM_HTIMER hTimer;
  TA_hWin = GUI_CreateDialogBox(_aDialogCreate, GUI_COUNTOF(_aDialogCreate), _cbDialog, WM_HBKWIN, 0, 0);
	hTimer =WM_CreateTimer(WM_GetClientWindow(TA_hWin),0,100,0);
  (void)hTimer;
  return TA_hWin;
}
