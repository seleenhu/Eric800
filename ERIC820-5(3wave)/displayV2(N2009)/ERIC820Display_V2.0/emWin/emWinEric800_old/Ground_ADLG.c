...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
  ...
  { BUTTON_CreateIndirect, "Resonance Alarm", ID_BUTTON_8, 0, 150, 120, 39, 0, 0x0, 0 },
	{ BUTTON_CreateIndirect, "Short Alarm", ID_BUTTON_11, 680, 110, 120, 39, 0, 0x0, 0 },
  ...
  { BUTTON_CreateIndirect, "Up", ID_BUTTON_9, 265, 440, 80, 35, 0, 0x0, 0 },
  { BUTTON_CreateIndirect, "Next", ID_BUTTON_10, 465, 440, 80, 35, 0, 0x0, 0 },
	{ BUTTON_CreateIndirect, "Reset", ID_BUTTON_12, 335, 120, 80, 35, 0, 0x0, 0 },
  ...
  { TEXT_CreateIndirect, "", ID_TEXT_0, 175, 75, 220, 20, 0, 0x0, 0 },
  { TEXT_CreateIndirect, "", ID_TEXT_1, 175, 120, 50, 25, 0, 0x0, 0 },
  { TEXT_CreateIndirect, "", ID_TEXT_2, 175, 160, 200, 20, 0, 0x0, 0 },
  { TEXT_CreateIndirect, "", ID_TEXT_3, 500, 160, 200, 20, 0, 0x0, 0 },
  { TEXT_CreateIndirect, "", ID_TEXT_4, 175, 200, 200, 20, 0, 0x0, 0 },
	{ TEXT_CreateIndirect, "", ID_TEXT_9, 500, 200, 200, 20, 0, 0x0, 0 },
	{ TEXT_CreateIndirect, "", ID_TEXT_6, 235, 120, 100, 25, 0, 0x0, 0 },
...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  switch (pMsg->MsgId) {
    ...
    //
		#ifdef Use_AllData 
		PhaseFlag=SDSuddentAlarmData->ValueFlag;
		#else
		...
		else
 {
    ...
    TEXT_SetTextColor(hItem, GUI_WHITE);
		//VoltageTemp=SystemAlarm->GroundAlarm.UcValue;
		#ifdef Use_AllData 
		Voltage=SDSuddentAlarmData->RMSValue[7][1];	
		VoltageTemp=Voltage;
		#else		
		OffSet++;
		Voltage=*(u8 *)(SDAlarm_SDRAM+OffSet);
		...
		for(i=6;i<18;i++)//中心点在第10周期 前4后8
			{
				...
				while(1)
				{
					...
					#else
					Wave[0]=*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+2*n);
					Wave[0]+=(*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+2*n+1))<<8;
					
					Wave[1]=*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+10240+2*n);
					Wave[1]+=(*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+10240+2*n+1))<<8;
					
					Wave[2]=*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+20480+2*n);
					Wave[2]+=(*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+20480+2*n+1))<<8;					
					#endif
		...
		for(i=6;i<18;i++)//中心点在第10周期 前4后8
			{
				...
				while(1)
				{
					...
					#else
					Wave[3]=*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+10240*3+2*n);
					Wave[3]+=(*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+10240*3+2*n+1))<<8;
					#endif					
		...
		for(i=6;i<18;i++)//中心点在第10周期 前4后8
			{
				...
				while(1)
				{
					...
					#else
					Wave[4]=*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+10240*4+2*n);
					Wave[4]+=(*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+10240*4+2*n+1))<<8;
					
					Wave[5]=*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+10240*5+2*n);
					Wave[5]+=(*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+10240*5+2*n+1))<<8;
					
					Wave[6]=*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+10240*6+2*n);
					Wave[6]+=(*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+10240*6+2*n+1))<<8;
					#endif
		...
		for(i=6;i<18;i++)//中心点在第10周期 前4后8
			{
				...
				while(1)
				{
					...
					#else
					Wave[7]=*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+10240*7+2*n);
					Wave[7]+=(*(u8 *)(SDAlarm_SDRAM+G_VoltageDataOffset+i*512+10240*7+2*n+1))<<8;
					#endif					
  ...
  case WM_PAINT:
    ...
    GUI_MEMDEV_Delete(hMemJPEG);//
		...
		#endif
		//=======标X轴刻度
				GUI_DispStringAt("-80ms",20,420);
				GUI_DispStringAt("-40ms",276-128,420);
				GUI_DispStringAt("0",276,420);
				GUI_DispStringAt("40ms",276+128,420);
				GUI_DispStringAt("80ms",276+256,420);
				GUI_DispStringAt("120ms",276+384,420);
