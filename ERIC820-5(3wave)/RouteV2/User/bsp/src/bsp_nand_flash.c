...
#if 0
	buf[0] = *(__IO uint8_t *)(Bank_NAND_ADDR | DATA_AREA);
	buf[1] = *(__IO uint8_t *)(Bank_NAND_ADDR | (DATA_AREA+1));
	buf[2] = *(__IO uint8_t *)(Bank_NAND_ADDR | (DATA_AREA+2));
	buf[3] = *(__IO uint8_t *)(Bank_NAND_ADDR | (DATA_AREA+3));
	buf[4] = *(__IO uint8_t *)(Bank_NAND_ADDR | (DATA_AREA+4));
	data = buf[0]<<24;
	data += buf[1]<<16;
	data += buf[2]<<8;
	data += buf[3];
...
uint8_t NAND_GetBlockInfo(NAND_BLOCK_INFO_T *_pInfo)
{
	...
	{
		_pInfo->ChipID = W29N01H;
		strcpy(_pInfo->ChipName, "W29N01H");
	}
	else if(id == W29N02H)
	{
		_pInfo->ChipID = W29N02H;
		strcpy(_pInfo->ChipName, "W29N02H");
	}
	else
	{		
		return 0;
