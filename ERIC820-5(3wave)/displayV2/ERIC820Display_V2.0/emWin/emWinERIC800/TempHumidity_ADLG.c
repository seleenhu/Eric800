...
#define ID_LISTVIEW_0     (GUI_ID_USER + 0x15)
...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
 ...
 // { BUTTON_CreateIndirect, "Button", ID_BUTTON_4, 670, 0, 129, 45, 0, 0x0, 0 },
  ...
  { BUTTON_CreateIndirect, "Resonance Alarm", ID_BUTTON_8, 0, 431, 132, 45, 0, 0x0, 0 },
	...
	{ BUTTON_CreateIndirect, "PTOpen Alarm", ID_BUTTON_10, 598, 431, 91, 45, 0, 0x0, 0  },
  ...
  { TEXT_CreateIndirect, "Temperture 95 ", ID_TEXT_2, 441, 199, 151, 25, 0, 0x0, 0 },
	{ LISTVIEW_CreateIndirect, "Listview", ID_LISTVIEW_0, 0, 46, 800, 385, 0, 0x0, 0 },
	//=====displaytime
...
#endif
extern uint8_t RFRXJudge(CupboardParameter CupboardData,float *TemData,uint8_t *HemData);
/*********************************************************************
*
*       Static code
*
...
static void CheckTempHumdityAlarm(WM_MESSAGE * pMsg_A)
{
	...
	for(i=0;i<20;i++){
		for(j=0;j<11;j++){
			if(RFRXJudge(Cupboard_P[i][j],&TemValue, &HemValue)){
				...
				LISTVIEW_SetItemText(hItem, 2,m,DisplayHZbuf[j] );
				if(j<10){
					LISTVIEW_SetItemText(hItem, 3,m,"温度" );
					sprintf(s,"%6.2f",TemValue);
				}else{
					LISTVIEW_SetItemText(hItem, 3,m,"湿度" );
					sprintf(s,"%d%%",HemValue);
				}				
		...
		rt = RFRXJudge(Cupboard_P[i][j],&TemValue, &HemValue);
		switch(rt){
			case 3:
				sprintf(s,"%d",m+1);
			...
			case 9:
				...
				sprintf(s,"A%d",i+1);
				LISTVIEW_SetItemText(hItem, 1,m,s);
				LISTVIEW_SetItemText(hItem, 2,m,"漏电流" );
				LISTVIEW_SetItemText(hItem, 3,m,"电流" );
				sprintf(s,"%d",Cupboard_P[i][j].Value.ProtectorData.LeakgaeCurrent);
				LISTVIEW_SetItemText(hItem, 4,m, s);
				//======显示时间
				year=Cupboard_P[i][j].TimeStamp.year;
				month=Cupboard_P[i][j].TimeStamp.month;
	...
	for(i=0;i<SystemSet.CupboardSIZE;i++)
		{	
			...
			j=11;
			if(Cupboard_P[i][j].OnlineFlag){
				if(Cupboard_P[i][j].Value.ProtectorData.LeakgaeCurrent > Setting.LeakageCurrentSet.LeakageValue)
				{
					LISTVIEW_AddRow(hItem, NULL);
					LISTVIEW_SetFont(hItem,&XBF_Font24);
					LISTVIEW_SetTextColor(hItem,LISTVIEW_CI_UNSEL,GUI_RED);
...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  case WM_INIT_DIALOG:
    ...
    LISTVIEW_AddColumn(hItem, 200, "位置", GUI_TA_HCENTER | GUI_TA_VCENTER);
    LISTVIEW_AddColumn(hItem, 80, "类型", GUI_TA_HCENTER | GUI_TA_VCENTER);
    LISTVIEW_SetGridVis(hItem, 1);
    LISTVIEW_AddColumn(hItem, 100, "超标值", GUI_TA_HCENTER | GUI_TA_VCENTER);
    LISTVIEW_AddColumn(hItem, 270, "时间", GUI_TA_HCENTER | GUI_TA_VCENTER);
