...
#if 0
	if((0 == ADCParameter.InstantUADCFlag) && (ADCParameter.flag & (1<<JudgePutOPBit))){
		if( 0 == ADCParameter.ADCFaskFlag){
			for(i=0;i<4;i++){
				if((sNowAdc[i] > ADCParameter.InstantUADC[i])||(sNowAdc[i] < -ADCParameter.InstantUADC[i])){
				//	Test1PinSet();
					ADCParameter.InstantUADCFlag |= (1<<ADCCalculateBit);
					ADCFifo->Instantbt = ADCFifo->pt;
					ADCFifo->Calbt = ADCFifo->pt;
					FaultFifo[FaultFifo_pt]->Tim = SysGetTime();
					FaultFifo[FaultFifo_pt]->FaultStyle |= (1<<i);				
					break;
				}
			}
		}else{
			if(ADCParameter.ADCFaskGndTime < 65000){
				ADCParameter.ADCFaskGndTime++;
				if((FaskFaultArcGnd == ADCParameter.ADCFaskFlag)&&( 0 == ADCParameter.ADCFaskArcGndTimes)){
					if((4 * 12800 + 6400) == ADCParameter.ADCFaskGndTime){						
							SystemState.SwitchState.AphaseState = 0x02;
							SystemState.SwitchState.BphaseState = 0x02;
							SystemState.SwitchState.CphaseState = 0x02;
					}
					if((5 * 12800) == ADCParameter.ADCFaskGndTime){
						ADCParameter.InstantUADCFlag |= (1<<ADCCalculateBit);
						ADCFifo->Instantbt = ADCFifo->pt;
						ADCFifo->Calbt = ADCFifo->pt;
						FaultFifo[FaultFifo_pt]->Tim = SysGetTime();
						ADCParameter.ADCFaskArcGndTimes++;
...
void SaveWave(u16 bt, u16 len)
{
	...
	i = bt;				
	for(j=0;j<len;j++){	
		FaultFifo[FaultFifo_pt]->Wave[0][j] = (ADCFifo.UAFIFO)[i];		
		i = FIFO_pointer(i,1,ADCFifo.FifoSize);	
	}
	i = bt;				
	for(j=0;j<len;j++){	
		FaultFifo[FaultFifo_pt]->Wave[1][j] = (ADCFifo.UBFIFO)[i];		
		i = FIFO_pointer(i,1,ADCFifo.FifoSize);	
	}
	i = bt;				
	for(j=0;j<len;j++){	
		FaultFifo[FaultFifo_pt]->Wave[2][j] = (ADCFifo.UCFIFO)[i];		
		i = FIFO_pointer(i,1,ADCFifo.FifoSize);	
	}
	i = bt;				
	for(j=0;j<len;j++){	
		FaultFifo[FaultFifo_pt]->Wave[3][j] = (ADCFifo.U0FIFO)[i];		
		i = FIFO_pointer(i,1,ADCFifo.FifoSize);	
	}
	
	i = bt;				
	for(j=0;j<len;j++){	
		FaultFifo[FaultFifo_pt]->Wave[4][j] = (ADCFifo.IAFIFO)[i];		
		i = FIFO_pointer(i,1,ADCFifo.FifoSize);	
	}
	i = bt;				
	for(j=0;j<len;j++){	
		FaultFifo[FaultFifo_pt]->Wave[5][j] = (ADCFifo.IBFIFO)[i];		
		i = FIFO_pointer(i,1,ADCFifo.FifoSize);	
	}
	i = bt;				
	for(j=0;j<len;j++){	
		FaultFifo[FaultFifo_pt]->Wave[6][j] = (ADCFifo.ICFIFO)[i];		
		i = FIFO_pointer(i,1,ADCFifo.FifoSize);	
	}
	i = bt;				
	for(j=0;j<len;j++){	
		FaultFifo[FaultFifo_pt]->Wave[7][j] = (ADCFifo.I0FIFO)[i];		
		i = FIFO_pointer(i,1,ADCFifo.FifoSize);	
	}
	FaultFifo[FaultFifo_pt]->DislayFlag = 0xFF;
	FaultFifo_pt++;	
...
{
#if 1
	uint16_t i,j,bt_back;
	float rmsbuf[8];
	const u16 bufsize = 1024;

	if(0 != PutOP_LED_STATE()){
		return;
	}
	...
	for(i=0;i<8;i++){
		rmsbuf[i] = 0;
		for(j=0;j<bufsize;j++){
			rmsbuf[i] += (ADCWholeVar->VerifyBuf[i][j] * ADCWholeVar->VerifyBuf[i][j]);
		}
	}
	for(i=0;i<8;i++){
		rmsbuf[i] /= bufsize;
		rmsbuf[i] = sqrt(rmsbuf[i]);		
	}	
	for(i=0;i<4;i++){
		ADCFifo->rati0[i] = rmsbuf[i]/100;
	}
	for(;i<8;i++){
		ADCFifo->rati0[i] = rmsbuf[i]/5;
	}
	FFT_VerifyDisplay();
	...
	bt_back = FIFO_Backpointer(ADCFifo.pt,4*256,ADCFifo.FifoSize);		
	for(j=0;j<bufsize;j++){
		ADCWholeVar->VerifyBuf[0][j] = ADCFifo.UAFIFO[bt_back];
		ADCWholeVar->VerifyBuf[1][j] = ADCFifo.UBFIFO[bt_back];
		ADCWholeVar->VerifyBuf[2][j] = ADCFifo.UCFIFO[bt_back];
		ADCWholeVar->VerifyBuf[3][j] = ADCFifo.U0FIFO[bt_back];		
		ADCWholeVar->VerifyBuf[4][j] = ADCFifo.IAFIFO[bt_back];
		ADCWholeVar->VerifyBuf[5][j] = ADCFifo.IBFIFO[bt_back];
		ADCWholeVar->VerifyBuf[6][j] = ADCFifo.ICFIFO[bt_back];
		ADCWholeVar->VerifyBuf[7][j] = ADCFifo.I0FIFO[bt_back];	
		bt_back = FIFO_pointer(bt_back,1,ADCFifo.FifoSize);
	}
	for(i=0;i<8;i++){
		rmsbuf[i] = 0;
	...
	for(i=0;i<8;i++)
	{
		g_tParam.fftratio[i]=ADCFifo.fftratio[i];
		g_tParam.rati0[i]=ADCFifo.fftratio[i];
		//g_tParam.offsetV[i]=ADCFifo.offsetV[i];
	}
	SaveParam();
#endif	
