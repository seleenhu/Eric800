...
#define FLASH_SECTOR62_ADDR             (0x0007C000u)
...
void EepromDataInit(void)
{
	#if 0
  uint32_t u32Addr;
	u32Addr=FLASH_SECTOR62_ADDR;
	memcpy((uint8_t *)(&EepromData),(volatile uint32_t*)FLASH_SECTOR62_ADDR,sizeof(EepromStruct));
   ...
   while(Set != EFM_GetFlagStatus(EFM_FLAG_RDY));
	 ...
	 memset((uint8_t *)&buf,0,128);
	 
   EFM_ReadByte32(FLASH_SECTOR62_ADDR,(sizeof(EepromStruct)/4+1)*4,(uint32_t *)buf);
	 
	 memcpy((uint8_t *)(&EepromData),buf,sizeof(EepromStruct));
...
uint8_t FlashWrite(void)
{
   ...
   EFM_SectorErase(FLASH_SECTOR62_ADDR);

   u32Addr=FLASH_SECTOR62_ADDR;
	 pt=(uint32_t *)(&EepromData);
   for( i = 0; i < (sizeof(EepromStruct)/4+1); i++)
    {
			
      EFM_SingleProgram(u32Addr,pt[i]);
      u32Addr += 4u;
     }
