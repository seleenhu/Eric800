...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
  ...
  { TEXT_CreateIndirect, "", ID_TEXT_0, 175, 75, 250, 20, 0, 0x0, 0 },
...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  switch (pMsg->MsgId) {
    ...
    //TEXT_SetTextColor(hItem, GUI_MAKE_COLOR(0x00000000));
		#ifdef Use_AllData
		year=SDAlarmData->ShortAlarm.Tim.year;
		month=SDAlarmData->ShortAlarm.Tim.month;
		day=SDAlarmData->ShortAlarm.Tim.day;
		hour=SDAlarmData->ShortAlarm.Tim.hour;
		minute=SDAlarmData->ShortAlarm.Tim.minute;
		second=SDAlarmData->ShortAlarm.Tim.second;		
		//us=SDAlarmData->ShortAlarm.Tim.us;		
		//us=us/1000;	 
		#else	 
    ...
    //
		...
		TEXT_SetTextColor(hItem, GUI_WHITE); 
		TEXT_SetFont(hItem, &XBF_Font24);
		sprintf(s,"全部故障:%d",SDShortFault_sum);
		TEXT_SetText(hItem,s);
		
		hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_8);
		TEXT_SetTextColor(hItem, GUI_WHITE); 
		TEXT_SetFont(hItem, &XBF_Font24);
		sprintf(s,"当前故障:%d",SDShortFault_sum-SA_WavePosition);
		TEXT_SetText(hItem,s);
    ...
    //TEXT_SetTextColor(hItem, GUI_MAKE_COLOR(0x0000FFFF));
		...
		#ifdef Use_AllData 
		Voltage=SDAlarmData->ShortAlarm.RMSValue[0];	
		#else		
    ...
    TEXT_SetTextColor(hItem, GUI_GREEN);
		...
		#ifdef Use_AllData 
		Voltage=SDAlarmData->ShortAlarm.RMSValue[1];	
		#else	
    ...
    TEXT_SetTextColor(hItem, GUI_RED);
		...
		#ifdef Use_AllData 
		Voltage=SDAlarmData->ShortAlarm.RMSValue[2];	
		#else	
    ...
    TEXT_SetTextColor(hItem, GUI_WHITE);
		//VoltageTemp=SystemAlarm->GroundAlarm.UcValue;
		#ifdef Use_AllData 
		Voltage=SDAlarmData->ShortAlarm.RMSValue[3];	
		#else	
		...
		else
 {
    ...
    //TEXT_SetTextColor(hItem, GUI_MAKE_COLOR(0x0000FFFF));
		//VoltageTemp=SystemAlarm->GroundAlarm.UaValue;
		#ifdef Use_AllData 
		Voltage=SDAlarmData->ShortAlarm.RMSValue[4];	
		#else	
    ...
    TEXT_SetTextColor(hItem, GUI_GREEN);
		//VoltageTemp=SystemAlarm->GroundAlarm.UbValue;
		#ifdef Use_AllData 
		Voltage=SDAlarmData->ShortAlarm.RMSValue[5];	
		#else	
    ...
    TEXT_SetTextColor(hItem, GUI_RED);
		//VoltageTemp=SystemAlarm->GroundAlarm.UcValue;
		#ifdef Use_AllData 
		Voltage=SDAlarmData->ShortAlarm.RMSValue[6];	
		#else	
    ...
    TEXT_SetTextColor(hItem, GUI_WHITE);
		//VoltageTemp=SystemAlarm->GroundAlarm.UcValue;
		#ifdef Use_AllData 
		Voltage=SDAlarmData->ShortAlarm.RMSValue[7];	
		#else	
		...
		for(i=6;i<18;i++)//中心点在第10周期 前4后8
			{
				...
				{
					#ifdef Use_AllData 
					Wave[0]=*(u16 *)(&(SDAlarmData->ShortAlarm.Wave[0][0])+i*256+n);
					Wave[1]=*(u16 *)(&(SDAlarmData->ShortAlarm.Wave[1][0])+i*256+n);
					Wave[2]=*(u16 *)(&(SDAlarmData->ShortAlarm.Wave[2][0])+i*256+n);
					#else
					Wave[0]=*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+2*n);
					Wave[0]+=(*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+2*n+1))<<8;
					
					Wave[1]=*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+10240+2*n);
					Wave[1]+=(*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+10240+2*n+1))<<8;
					
					Wave[2]=*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+20480+2*n);
					Wave[2]+=(*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+20480+2*n+1))<<8;
					#endif
		...
		for(i=6;i<18;i++)//中心点在第10周期 前4后8
			{
				...
				{
					#ifdef Use_AllData 
					Wave[3]=*(u16 *)(&(SDAlarmData->ShortAlarm.Wave[3][0])+i*256+n);
					#else
					Wave[3]=*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+10240*3+2*n);
					Wave[3]+=(*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+10240*3+2*n+1))<<8;
					#endif
		...
		for(i=6;i<18;i++)//中心点在第10周期 前4后8
			{
				...
				{
					#ifdef Use_AllData 
					Wave[4]=*(u16 *)(&(SDAlarmData->ShortAlarm.Wave[4][0])+i*256+n);
					Wave[5]=*(u16 *)(&(SDAlarmData->ShortAlarm.Wave[5][0])+i*256+n);
					Wave[6]=*(u16 *)(&(SDAlarmData->ShortAlarm.Wave[6][0])+i*256+n);
					#else
					Wave[4]=*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+10240*4+2*n);
					Wave[4]+=(*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+10240*4+2*n+1))<<8;
					
					Wave[5]=*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+10240*5+2*n);
					Wave[5]+=(*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+10240*5+2*n+1))<<8;
					
					Wave[6]=*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+10240*6+2*n);
					Wave[6]+=(*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+10240*6+2*n+1))<<8;
					#endif
		...
		for(i=6;i<18;i++)//中心点在第10周期 前4后8
			{
				...
				{
					#ifdef Use_AllData 
					Wave[7]=*(u16 *)(&(SDAlarmData->ShortAlarm.Wave[7][0])+i*256+n);
					#else
					Wave[7]=*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+10240*7+2*n);
					Wave[7]+=(*(u8 *)(SDAlarm_SDRAM+S_VoltageDataOffset+i*512+10240*7+2*n+1))<<8;
					#endif
	...
	/*
		...
		TEXT_SetTextColor(hItem, GUI_WHITE); 
		TEXT_SetFont(hItem, GUI_FONT_24B_ASCII);
		sprintf(s,"No:%d",AlarmSaCount-SA_WavePosition);
		TEXT_SetText(hItem,s);
	*/
   break;
#endif	 
     ...
     GRAPH_AttachData(hItem,ahData3U);
		...
		for(i=0;i<4;i++)
		{
			...
			{	
			 ...
			 GRAPH_DATA_YT_AddValue(ahDataUA,SystemAlarm->GroundAlarm.Wave[0][i*256+n]/2+90);
				
			 
			 GRAPH_DATA_YT_AddValue(ahDataUB,SystemAlarm->GroundAlarm.Wave[1][i*256+n]/2+90);
				
			 GRAPH_DATA_YT_AddValue(ahDataUC,SystemAlarm->GroundAlarm.Wave[2][i*256+n]/2+90);
			}
			//Graph_Pt+=i*256;
		}
  ...
  case WM_NOTIFY_PARENT:
    ...
    case ID_BUTTON_8: // Notifications sent by 'Resonance Alarm'
      ...
      break;
			...
			case ID_BUTTON_21: // Notifications sent by 'PTOpen Alarm'
      switch(NCode) {
      case WM_NOTIFICATION_CLICKED:
        // USER START (Optionally insert code for reacting on notification message)
				Clear_Graph();
				GUI_EndDialog(pMsg->hWin,0);
        CreateUnderVoltage_A();//PT断线报警
        // USER END
        break;
      case WM_NOTIFICATION_RELEASED:
        // USER START (Optionally insert code for reacting on notification message)
    ...
    case ID_BUTTON_10: // Notifications sent by 'Up'
      ...
      case WM_NOTIFICATION_CLICKED:
        // USER START (Optionally insert code for reacting on notification message)
				if(SA_WavePosition>=(SDShortFault_sum-1))
				{
					break;
				}
				SA_WavePosition++;
    ...
    case ID_BUTTON_11: // Notifications sent by 'Next'
      ...
      case WM_NOTIFICATION_CLICKED:
        // USER START (Optionally insert code for reacting on notification message)
			  ...
			  if(SelectWave>=4)
				SelectWave=0;	
				WM_SendMessageNoPara(WM_GetClientWindow(SA_hWin), WM_DisplayWave);
...
*       CreateOverVoltage_A
*/
WM_HWIN CreateShort_A(void);
WM_HWIN CreateShort_A(void) {
  //WM_HWIN hWin;

  SA_hWin = GUI_CreateDialogBox(_aDialogCreate, GUI_COUNTOF(_aDialogCreate), _cbDialog, WM_HBKWIN, 0, 0);
  return SA_hWin;
}
