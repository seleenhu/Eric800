...
//	u8 rt;
	uint8_t i;
	int16_t sNowAdc[8];
	static uint32_t SiliconControlTim;
	//uint16_t bt;
	//ADCParameterSet();
	//TestPinXor();
...
#if 0
	SiliconControlTim++;
	if(SiliconControlTim < 2*256){		
		SiliconControl_flag = 1;			
	}else if(SiliconControlTim < (4*128*100 + 2*256)){
...
//======Test
	...
	if(SiliconControl_flag){

#if	0	
		SiliconControl_XOR();
		
		
		if(SiliconControlTim<128){
			SiliconControlTim++;
		}else{
			SiliconControlTim = 0;
			SiliconControl_flag = 0;
...
void ADCVerification(void)
{
	...
	memcpy(PoramData.ADCParameter.ratio,ADCFifo->rati0,8*sizeof(float));
	memcpy(PoramData.ADCParameter.fftratio,ADCFifo->fftratio,8*sizeof(float));
	PoramFlag++;
