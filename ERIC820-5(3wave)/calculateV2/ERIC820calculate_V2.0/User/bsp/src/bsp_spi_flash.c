...
//======add  extend address command
#define CMD_REG3_R			0x15   /*读Register 3*/
#define CMD_REG3_W			0x11   /*写Register 3*/
#define CMD_ExtAdd			0xc5		/*写扩展地址 */
#define CMD_ADD4				0xb7		/*4地址模式*/
...
/*
uint8_t TestBuf[10]={0xff,0x55,0xff,0x55,0x11,0x22,0x23,0x33,0x44,0x55};
uint8_t RevBuf[10];
*/
/*
*********************************************************************************************************
...
/*
*********************************************************************************************************
*	函 数 名: sf_SetAddressMode
*	功能说明: 初始化串行Flash ADP=0 3位地址模式  ADP=1 4位地址模式
*	形    参:  无
*	返 回 值: 无
*********************************************************************************************************
*/
void sf_SetAddressMode()
{
	uint8_t Value;
	sf_SetCS(0);									/* 使能片选 */
  bsp_spiWrite1(CMD_REG3_R);							/* 发送命令， 读状态寄存器 */
	Value=bsp_spiRead1();
	sf_SetCS(1);									/* 禁能片选 */
	...
	sf_SetCS(0);									/* 使能片选 */
	bsp_spiWrite1(CMD_REG3_W);							/* 发送命令， 写状态寄存器 */
	//Value&=0xfd; //S17 ADP=0 3位地址模式 
	Value|=0x02; //S17 ADP=1 4位地址模式 
	bsp_spiWrite1(Value);							/* 发送数据：状态寄存器的值 */
	sf_SetCS(1);	/* 禁能片选 */
	
	
	
	sf_SetCS(0);									/* 使能片选 */
  bsp_spiWrite1(CMD_ADD4);							/* 发送命令， 4地址模式*/
	sf_SetCS(1);									/* 禁能片选 */
...
void bsp_InitSFlash(void)
{
	...
	//=======Test
	#if 0
	sf_WriteBuffer(TestBuf,0,10);
	
	sf_ReadBuffer(RevBuf,0,10);
...
void sf_EraseSector(uint32_t _uiSectorAddr)
{
	...
	#if 0
	//====写扩展地址：
	bsp_spiWrite1(CMD_ExtAdd);
	bsp_spiWrite1((_uiSectorAddr & 0xFF000000) >> 24);	/* 发送扇区地址的扩展地址 */
	#endif 
	bsp_spiWrite1(CMD_SE);								/* 发送擦除命令 */
	bsp_spiWrite1((_uiSectorAddr & 0xFF000000) >> 24);
...
void sf_PageWrite(uint8_t * _pBuf, uint32_t _uiWriteAddr, uint16_t _usSize)
{
	...
	if (g_tSF.ChipID == SST25VF016B_ID)
	{
		...
		//====写扩展地址：
		bsp_spiWrite1(CMD_ExtAdd);
		bsp_spiWrite1((_uiWriteAddr & 0xFF000000) >> 24);	/* 发送扇区地址的扩展地址 */
		#endif 
	//	bsp_spiWrite1(CMD_AAI);							/* 发送AAI命令(地址自动增加编程) */
		bsp_spiWrite1((_uiWriteAddr & 0xFF000000) >> 24);
...
//====写扩展地址：
	bsp_spiWrite1(CMD_ExtAdd);
	bsp_spiWrite1((_uiReadAddr & 0xFF000000) >> 24);	/* 发送扇区地址的扩展地址 */	
	#endif
	bsp_spiWrite1(CMD_READ);							/* 发送读命令 */
	bsp_spiWrite1((_uiReadAddr & 0xFF000000) >> 24);
...
//====写扩展地址：
	bsp_spiWrite1(CMD_ExtAdd);
	bsp_spiWrite1((_uiSrcAddr & 0xFF000000) >> 24);	/* 发送扇区地址的扩展地址 */	
	#endif
	bsp_spiWrite1(CMD_READ);							/* 发送读命令 */
	bsp_spiWrite1((_uiSrcAddr & 0xFF000000) >> 24);
...
void sf_ReadInfo(void)
{
	/* 自动识别串行Flash型号 */
	{
		...
		switch (g_tSF.ChipID)
		{
			...
			case W25Q256FV_ID:
				strcpy(g_tSF.ChipName, "W25Q256FV");
