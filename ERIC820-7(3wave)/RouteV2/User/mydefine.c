...
void KeyScan(void)
{
	...
	if(GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_0)){
		...
		if(scantims<3){
			...
			//ModuleGlobal->SPI2BufTxLen = sprintf((char *)(ModuleGlobal->SPI2Buf),"This is UART7test!!\r\n");
			//ModuleSPITxStart(ModuleGlobal->SPI2BufTxLen);
...
void ReadParam(uint16_t PARAM_VERData)
{
	...
	if(PARAM_VERData == PoramData.ParamVer){
		...
		for(i=0;i<16;i++){
			ADCFifo->fftratio[i] = 2015.54*2;
			PoramData.ADCParameter.fftratio[i] = 2015.54*2;
		}
	}
