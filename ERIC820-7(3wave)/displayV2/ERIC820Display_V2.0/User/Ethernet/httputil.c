...
static void make_basic_config_setting_json_callback(int8* buf, CONFIG_MSG config_msg)
{
    ...
    sprintf(buf,"settingsCallback({\"ver\":\"%d.%d\",\
            ...
            Eepromdata.mac[0],Eepromdata.mac[1],Eepromdata.mac[2],Eepromdata.mac[3],Eepromdata.mac[4],Eepromdata.mac[5],
            Eepromdata.lip[0],Eepromdata.lip[1],Eepromdata.lip[2],Eepromdata.lip[3],
            Eepromdata.gw[0],Eepromdata.gw[1],Eepromdata.gw[2],Eepromdata.gw[3],
            Eepromdata.sub[0],Eepromdata.sub[1],Eepromdata.sub[2],Eepromdata.sub[3]
           );	
}
#else
static void make_basic_config_setting_json_callback(int8* buf)
{
	uint8 sw_ver[2] = {1,0};
    sprintf(buf,"settingsCallback({\"ver\":\"%d.%d\",\
                \"mac\":\"%02X:%02X:%02X:%02X:%02X:%02X\",\
...
void Https_Task(SOCKET s)
{
    ...
    case SOCK_ESTABLISHED:								/*socket处于连接状态*/
        ...
        {
            setSn_IR(s, Sn_IR_CON);						/*清除中断标志位*/
        }
        if ((len = getSn_RX_RSR(s)) > 0)
        {
            len = recv(s, (uint8*)http_request, len); 	/*接收http请求*/
            *(((uint8*)http_request)+len) = 0;
            proc_http(s, (uint8*)http_request);			/*接收http请求并发送http响应*/
            disconnect(s);
        }
        break;
    ...
    case SOCK_CLOSED:                   				/*socket处于关闭状态*/
        socket(s, Sn_MR_TCP, 80, 0x00);   				/*打开socket*/
        break;
...
void cgi_ipconfig(st_http_request *http_request)
{
    ...
    param = get_http_param_value(http_request->URI,"mac");			/*获取修改后的IP地址*/
    if(param)
    {
    	//NetDBG("mac = %s\r\n",param);
        inet_mac_((uint8*)param, ConfigMsg.mac);
...
#if 0	
    if(Save_NetCfg(&ConfigMsg))										/*将获取的网络参数写进eeprom*/
        NetDBG("Save_NetCfg to flash success !\r\n");
    else
        NetDBG("Save_NetCfg to flash fail !\r\n");		
#endif	
}
