...
void bsp_DetectLcdType(void)
{
	...
	for (i = 0; i < 5; i++)
	{
		...
		*/
		...
		#endif
		if (i2c_CheckDevice(NS2009_I2C_ADDRESS) == 0)
		{
			g_TouchType = CT_NS2009;	/* 触摸类型 */
			g_LcdType = LCD_70_800X480;
			NS2009_InitHard();	/* 必须先配置 */
			break;
...
void TOUCH_PutKey(uint8_t _ucEvent, uint16_t _usX, uint16_t _usY)
{
	...
	switch (SystemSet.TouchDirection)
	{
		case 0:	/* 校准触摸时，屏幕方向为0 */
...
{
#if 1
	SystemSet.usAdcX1 = g_tTP.usAdcX1;
	SystemSet.usAdcY1 = g_tTP.usAdcY1;
	SystemSet.usAdcX2 = g_tTP.usAdcX2;
	SystemSet.usAdcY2 = g_tTP.usAdcY2;
	SystemSet.usAdcX3 = g_tTP.usAdcX3;
	SystemSet.usAdcY3 = g_tTP.usAdcY3;
	SystemSet.usAdcX4 = g_tTP.usAdcX4;
	SystemSet.usAdcY4 = g_tTP.usAdcY4;

	SystemSet.usLcdX1 = g_tTP.usLcdX1;
	SystemSet.usLcdY1 = g_tTP.usLcdY1;
	SystemSet.usLcdX2 = g_tTP.usLcdX2;
	SystemSet.usLcdY2 = g_tTP.usLcdY2;
	SystemSet.usLcdX3 = g_tTP.usLcdX3;
	SystemSet.usLcdY3 = g_tTP.usLcdY3;
	SystemSet.usLcdX4 = g_tTP.usLcdX4;
	SystemSet.usLcdY4 = g_tTP.usLcdY4;

	SystemSet.XYChange = g_tTP.XYChange;

	SystemSet.TouchDirection = g_LcdDirection;	/* 2014-09-11 添加屏幕方向, 用于屏幕旋转时无需再次校准 */
