...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
 ...
 { BUTTON_CreateIndirect, "Over Voltage Alarm", ID_BUTTON_5, 378, 431, 107, 45, 0, 0x0, 0 },
  ...
  { BUTTON_CreateIndirect, "Resonance Alarm", ID_BUTTON_8, 0, 431, 132, 45, 0, 0x0, 0 },
	{ BUTTON_CreateIndirect, "Short Alarm", ID_BUTTON_11, 691, 431, 109, 45, 0, 0x0, 0 },
  ...
  { BUTTON_CreateIndirect, "Up", ID_BUTTON_9, 50, 380, 130, 45, 0, 0x0, 0  },
  { BUTTON_CreateIndirect, "Next", ID_BUTTON_10, 240, 380, 130, 45, 0, 0x0, 0 },
	{ BUTTON_CreateIndirect, "Reset", ID_BUTTON_12, 620, 380,130, 45, 0, 0x0, 0 },
  ...
  { TEXT_CreateIndirect, "", ID_TEXT_4, 620, 94, 200, 20, 0, 0x0, 0 },
	...
	#if 0
  { TEXT_CreateIndirect, "3U0=0.25          KV", ID_TEXT_5, 500, 197, 176, 20, 0, 0x0, 0 },
	#endif
...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  switch (pMsg->MsgId) {
    ...
    //TEXT_SetFont(hItem, GUI_FONT_20B_ASCII);
		...
		if(4 != SelectGndWave){
			switch(PhaseFlag&0x7f)
			{
				...
				case 2:
					...
					break;
				case 3:
					TEXT_SetText(hItem,"AB相");
					break;
				case 4:
					TEXT_SetText(hItem,"C相");
					break;
				case 5:
					TEXT_SetText(hItem,"AC相");
					break;
				case 6:
					TEXT_SetText(hItem,"BC相");
					break;
				default:
					break;
			}
		...
		if(4 != SelectGndWave){
			if(PhaseFlag&0x80){
				TEXT_SetText(hItem,"弧光接地");
			}else{
				TEXT_SetText(hItem,"金属接地");	
			}
		}else{
			if(0 == ModuleCombuf->FaultAllSelComBuf.CanId[0]){
				sprintf(s,"零序接地");
		...
		TEXT_SetTextColor(hItem, GUI_WHITE); 
		TEXT_SetFont(hItem, &XBF_Font24);
		sprintf(s,"全部故障:%d",SDGroundFault_sum);
		TEXT_SetText(hItem,s);
		
		hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_8);
		TEXT_SetTextColor(hItem, GUI_WHITE); 
		TEXT_SetFont(hItem, &XBF_Font24);
		sprintf(s,"当前故障:%d",SDGroundFault_sum-GA_WavePosition);
		TEXT_SetText(hItem,s);
    ...
    //
		...
		switch(SelectGndWave){
			...
			case 1:	//display 3U0
				...
				Voltage=ModuleComFaultbuf->RMSValue[0][1];	
				VoltageTemp=Voltage;	
				...
				Voltage=ModuleComFaultbuf->RMSValue[1][1];	
				VoltageTemp=Voltage;
				...
				Voltage=ModuleComFaultbuf->RMSValue[2][1];	
				VoltageTemp=Voltage;
			...
			case 4:
				...
				hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_2);
				TEXT_SetFont(hItem, GUI_FONT_24B_ASCII);
				VoltageTemp=ModuleCombuf->FaultAllSelComBuf.RMSValue[0];
				Sort_D[0]=VoltageTemp*SystemSet.SelRatioCTBuf[0];
				sprintf(s,"I =%6.2fA",Sort_D[0]);
				TEXT_SetTextColor(hItem, GUI_RED);
				TEXT_SetText(hItem,s);
				...
				TEXT_SetFont(hItem, GUI_FONT_20B_ASCII);
				TEXT_SetTextColor(hItem, GUI_MAKE_COLOR(0x000000FF));
				TEXT_SetTextColor(hItem, 0x00333333);
				sprintf(s,"      ");
				TEXT_SetText(hItem,s);
				...
				Scale=VoltageTemp/(30/2);
			 
				for(i=0;i<12;i++)//中心点在第10周期 前4后8
				{
					
					for(n=0;n<64;n++){
						Wave[4] = ModuleCombuf->FaultAllSelComBuf.Wave[0][i*32+n/2];							
						GRAPH_DATA_YT_AddValue(ahDataUA,Wave[4]/(Scale*ModuleCombuf->FaultAllSelComBuf.ratio[0])+100);	
					}	
  ...
  case WM_PAINT:
		...
		//=======标X轴刻度
				...
				//======fresh wave
				if(SelectGndWave!=SelectWaveSave)
				{
					SelectWaveSave=SelectGndWave;
					WM_SendMessageNoPara(WM_GetClientWindow(GA_hWin), WM_DisplayWave);
				}
  ...
  case WM_NOTIFY_PARENT:
    ...
    case ID_BUTTON_8: // Notifications sent by 'Resonance Alarm'
      ...
      break;			
		...
		case ID_BUTTON_12: // Notifications sent by 'Resonance Alarm'
      switch(NCode) {
      case WM_NOTIFICATION_CLICKED:
        // USER START (Optionally insert code for reacting on notification message)
				//SystemAlarm->GroundAlarm.ResetFlag=1;
				ModuleCombuf->ResetFlag = 1;
        // USER END
        break;
      case WM_NOTIFICATION_RELEASED:
        // USER START (Optionally insert code for reacting on notification message)
    ...
    case ID_BUTTON_9: // Notifications sent by 'Up'
      switch(NCode) {
      case WM_NOTIFICATION_CLICKED:
        // USER START (Optionally insert code for reacting on notification message)
				if(GA_WavePosition>=(SDGroundFault_sum-1))
				{
					break;
				}
				GA_WavePosition++;
      ...
      break;
    case ID_BUTTON_10: // Notifications sent by 'Next'
      switch(NCode) {
      case WM_NOTIFICATION_CLICKED:
        // USER START (Optionally insert code for reacting on notification message)
				if(GA_WavePosition>0)
				{
					GA_WavePosition--;
					StartReadSD_GA_Flag=1;
				}
...
*       CreateOverVoltage_A
*/
WM_HWIN CreateGround_A(void);
WM_HWIN CreateGround_A(void) {
  //WM_HWIN hWin;
	WM_HTIMER hTimer;
  GA_hWin = GUI_CreateDialogBox(_aDialogCreate, GUI_COUNTOF(_aDialogCreate), _cbDialog, WM_HBKWIN, 0, 0);
	hTimer =WM_CreateTimer(WM_GetClientWindow(GA_hWin),0,100,0);
  (void)hTimer;
  return GA_hWin;
}
