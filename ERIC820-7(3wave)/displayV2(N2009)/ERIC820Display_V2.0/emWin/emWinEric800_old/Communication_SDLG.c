...
#define ID_DROPDOWN_1	(GUI_ID_USER + 0x16)
...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
  { FRAMEWIN_CreateIndirect, "Communication_s", ID_FRAMEWIN_0, 0, 0, 800, 480, 0, 0x0, 0 },
  ...
  { BUTTON_CreateIndirect, "Voltag Limit Set", ID_BUTTON_10, 0, 265, 150, 39, 0, 0x0, 0 },
	...
	#endif
  { EDIT_CreateIndirect, "Edit", ID_EDIT_0, 330, 140, 145, 40, 0, 0x64, 0 },
  //{ EDIT_CreateIndirect, "Edit", ID_EDIT_1, 330, 190, 220, 40, 0, 0x64, 0 },
	
	
  { EDIT_CreateIndirect, "Edit", ID_EDIT_2, 370, 310, 170, 40, 0, 0x64, 0 },
  { EDIT_CreateIndirect, "Edit", ID_EDIT_3, 370, 365, 170, 40, 0, 0x64, 0 },
	
	{ DROPDOWN_CreateIndirect, "Dropdown", ID_DROPDOWN_0, 330, 190, 145, 40, 0, 0x0, 0 },
	{ DROPDOWN_CreateIndirect, "SelectNet", ID_DROPDOWN_1, 650, 100, 100, 40, 0, 0x0, 0 },
...
static void GetIpAdress(char *buf)
{
	char TempBuffer[10];
	u8 i=0;
	u8 n=0;
  ...
  return;		
	...
	IpAdr[0]=atof(TempBuffer); // IPDAdr0  
...
static void _cbDialog(WM_MESSAGE * pMsg) {
  ...
  switch (pMsg->MsgId) {
	case WM_SetValue:
		...
		{
			case 0:
				Setting.RS485Set.adr=Rs485Adr;
			
				//g_tParam.Addr485=Rs485Adr;				
				//bsp_InitRS485();
				//SaveParam();
			
				SystemSet.RS485Set.adr = Rs485Adr;
				bsp_InitRS485();
				SystemSet.ParamSaveFlag = FlagSystemState;
				break;
			case 1:
				Setting.RS485Set.Baud=Rs485Baud;
				//g_tParam.Baud485=Rs485Baud;
				SystemSet.RS485Set.Baud = Rs485Baud;
				SystemSet.ParamSaveFlag = FlagSystemState;
				//SaveParam();
				break;
			case 2:
				//Setting.NetSet.IPadrr[0]=IpAdr[0];
				//Setting.NetSet.IPadrr[1]=IpAdr[1];
				//Setting.NetSet.IPadrr[2]=IpAdr[2];
				//Setting.NetSet.IPadrr[3]=IpAdr[3];
				//g_tParam.sip_ip[0]=IpAdr[0];
			 // g_tParam.sip_ip[1]=IpAdr[1];
				//g_tParam.sip_ip[2]=IpAdr[2];
				//g_tParam.sip_ip[3]=IpAdr[3];			
				//Init4G();
				//SaveParam();
				...
				Setting.NetSet.IPadrr[2]=IpAdr[2];
				Setting.NetSet.IPadrr[3]=IpAdr[3];
				SystemSet.NetSet.IPadrr[0] = IpAdr[0];
				SystemSet.NetSet.IPadrr[1] = IpAdr[1];
				SystemSet.NetSet.IPadrr[2] = IpAdr[2];
				SystemSet.NetSet.IPadrr[3] = IpAdr[3];
				//Init4G();
				SystemSet.ParamSaveFlag = FlagSystemState;
				break;
			case 3:
				//Setting.NetSet.Port=NetProt;
				//g_tParam.NetPort=NetProt;
				//Init4G();
				//SaveParam();
				
				Setting.NetSet.Port=NetProt;
				SystemSet.NetSet.Port =NetProt;
				//Init4G();
				SystemSet.ParamSaveFlag = FlagSystemState;
				break;
			default:
  ...
  case WM_PAINT:
    ...
    //GUI_SetFont(GUI_FONT_20B_ASCII);
		GUI_SetFont(&XBF_Font24);
    //GUI_DispStringAt("485Communication Set",210,100);
		GUI_DispStringAt("485通讯设置",210,100);
	
    GUI_SetColor(GUI_WHITE);
    //GUI_SetFont(GUI_FONT_20B_ASCII);
    //GUI_DispStringAt("Address",210,145);
		GUI_DispStringAt("通讯地址",210,145);
    //GUI_DispStringAt("BaudRate",210,200);
		GUI_DispStringAt("波特率",210,200);

    GUI_SetColor(GUI_GREEN);
    //GUI_SetFont(GUI_FONT_20B_ASCII);
    //GUI_DispStringAt("Internet Communication  Set",210,265);
		GUI_DispStringAt("以太网通讯设置",210,265);

    GUI_SetColor(GUI_WHITE);
    //GUI_SetFont(GUI_FONT_20B_ASCII);
    //GUI_DispStringAt("Sever IP Addr",210,315);
		GUI_DispStringAt("服务器IP地址",210,315);
    //GUI_DispStringAt("Sever COM",210,370);
		GUI_DispStringAt("服务器端口",210,370);
		
		
		GUI_SetColor(GUI_GREEN);
    GUI_SetFont(&XBF_Font24);
    //GUI_DispStringAt("Internet Communication  Set",210,265);
		GUI_DispStringAt("网络选择",530,100);
  ...
  case WM_INIT_DIALOG:
    //
    // Initialization of 'Communication_s'
    //
		SetIndex=1;
    ...
    hItem = WM_GetDialogItem(pMsg->hWin, ID_EDIT_0);
		//sprintf(Buffer,"%d",g_tParam.Addr485);		
		sprintf(Buffer,"%d",SystemSet.RS485Set.adr);
    ...
    hItem = WM_GetDialogItem(pMsg->hWin, ID_EDIT_1);
		sprintf(Buffer,"%d",g_tParam.Baud485);
   // EDIT_SetText(hItem, "115200");
    ...
    //DROPDOWN_SetAutoScroll(hItem,1);
		...
		//switch(g_tParam.Baud485)		
		switch(SystemSet.RS485Set.Baud)
		{
			case 9600:
    ...
    DROPDOWN_SetListHeight(hItem, 60);
		...
		DROPDOWN_SetTextAlign(hItem, GUI_TA_LEFT | GUI_TA_VCENTER);
		DROPDOWN_AddString(hItem, "4G");
		DROPDOWN_AddString(hItem, "有线");
		switch(Setting.NetSet.NetSelectFlag)
		{
			case 0:
				DROPDOWN_SetSel(hItem,0);
				break;
    ...
    hItem = WM_GetDialogItem(pMsg->hWin, ID_EDIT_2);
		...
		//sprintf(Buffer,"%d.%d.%d.%d",g_tParam.sip_ip[0],g_tParam.sip_ip[1],g_tParam.sip_ip[2],g_tParam.sip_ip[3]);
		sprintf(Buffer,"%d.%d.%d.%d",SystemSet.NetSet.IPadrr[0],SystemSet.NetSet.IPadrr[1],SystemSet.NetSet.IPadrr[2],SystemSet.NetSet.IPadrr[3]);		
    //EDIT_SetText(hItem, "192.168.1.1");
		EDIT_SetText(hItem, Buffer);
    EDIT_SetTextAlign(hItem, GUI_TA_LEFT | GUI_TA_VCENTER);
    EDIT_SetFont(hItem, GUI_FONT_24B_ASCII);
    //
    ...
    hItem = WM_GetDialogItem(pMsg->hWin, ID_EDIT_3);		
		//sprintf(Buffer,"%d",g_tParam.NetPort);
		sprintf(Buffer,"%d",SystemSet.NetSet.Port);
    //EDIT_SetText(hItem, "10000");
		EDIT_SetText(hItem, Buffer);
    EDIT_SetTextAlign(hItem, GUI_TA_LEFT | GUI_TA_VCENTER);
    EDIT_SetFont(hItem, GUI_FONT_24B_ASCII);
    ...
    case ID_EDIT_2: // Notifications sent by 'Edit'
      ...
      case WM_NOTIFICATION_CLICKED:
        // USER START (Optionally insert code for reacting on notification message)
				...
				SetType=2;//
				hItem = WM_GetDialogItem(pMsg->hWin, ID_EDIT_2);
				EDIT_GetText(hItem,Buffer,20);
				GetIpAdress(Buffer);//初始化数据
    ...
    // USER START (Optionally insert additional code for further Ids)
      ...
      case WM_NOTIFICATION_SEL_CHANGED:
        // USER START (Optionally insert code for reacting on notification message)
				...
				{
					case 0:
						Setting.RS485Set.Baud=9600;
						break;
					case 1:
						Setting.RS485Set.Baud=38400;
						break;
					case 2:
						Setting.RS485Set.Baud=115200;
						break;
					default:
						Setting.RS485Set.Baud=9600;
						break;
				...
				//g_tParam.Baud485=Setting.RS485Set.Baud;
				//SaveParam();
				SystemSet.RS485Set.Baud = Setting.RS485Set.Baud;
				SystemSet.ParamSaveFlag = FlagSystemState;
        // USER END
        break;
      // USER START (Optionally insert additional code for further notification handling)
      ...
      case WM_NOTIFICATION_SEL_CHANGED:
        // USER START (Optionally insert code for reacting on notification message)
				...
				{
					case 0:
						Setting.NetSet.NetSelectFlag=0;
						break;
					case 1:
						Setting.NetSet.NetSelectFlag=1;
						break;
				...
				//g_tParam.NetSelectFlag=Setting.NetSet.NetSelectFlag;
				//SaveParam();				
				SystemSet.ParamSaveFlag = FlagSystemState;
        // USER END
        break;
...
*/
WM_HWIN CreateCommunication_s(void);
WM_HWIN CreateCommunication_s(void) {
  //WM_HWIN hWin;

  CC_hWin = GUI_CreateDialogBox(_aDialogCreate, GUI_COUNTOF(_aDialogCreate), _cbDialog, WM_HBKWIN, 0, 0);
  return CC_hWin;
}
